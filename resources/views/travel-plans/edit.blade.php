                    ...
                    <form method="POST" action="{{ route('travel-plans.update', $travelPlan) }}">
                        ...
                        <div class="mb-6">
                            ...
                            <select name="timezone" id="timezone"
                                ...
                                <option value="Asia/Tokyo" {{ old('timezone', $travelPlan->timezone) == 'Asia/Tokyo' ? 'selected' : '' }}>日本時間 (Asia/Tokyo)</option>
                                <option value="Asia/Seoul" {{ old('timezone', $travelPlan->timezone) == 'Asia/Seoul' ? 'selected' : '' }}>韓国時間 (Asia/Seoul)</option>
                                <option value="Asia/Shanghai" {{ old('timezone', $travelPlan->timezone) == 'Asia/Shanghai' ? 'selected' : '' }}>中国時間 (Asia/Shanghai)</option>
                                <option value="Asia/Singapore" {{ old('timezone', $travelPlan->timezone) == 'Asia/Singapore' ? 'selected' : '' }}>シンガポール時間 (Asia/Singapore)</option>
                                <option value="Asia/Bangkok" {{ old('timezone', $travelPlan->timezone) == 'Asia/Bangkok' ? 'selected' : '' }}>タイ時間 (Asia/Bangkok)</option>
                                <option value="Europe/London" {{ old('timezone', $travelPlan->timezone) == 'Europe/London' ? 'selected' : '' }}>イギリス時間 (Europe/London)</option>
                                <option value="Europe/Paris" {{ old('timezone', $travelPlan->timezone) == 'Europe/Paris' ? 'selected' : '' }}>フランス時間 (Europe/Paris)</option>
                                <option value="America/New_York" {{ old('timezone', $travelPlan->timezone) == 'America/New_York' ? 'selected' : '' }}>アメリカ東部時間 (America/New_York)</option>
                                <option value="America/Los_Angeles" {{ old('timezone', $travelPlan->timezone) == 'America/Los_Angeles' ? 'selected' : '' }}>アメリカ西部時間 (America/Los_Angeles)</option>
                                <option value="Pacific/Honolulu" {{ old('timezone', $travelPlan->timezone) == 'Pacific/Honolulu' ? 'selected' : '' }}>ハワイ時間 (Pacific/Honolulu)</option>
                            </select>
                            @error('timezone')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
